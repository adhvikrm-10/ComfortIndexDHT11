...
String humidityBar(float h) {
  ...
  bar += "] ";
  if ((int)h < 10)  bar += "  ";
  else if ((int)h < 100) bar += " ";
  bar += String((int)h) + "%";
  return bar;
}

void divider(bool thick) { (void)thick; }
void dividerClose() {}
String uptime() {
...
void setup() {
  ...
  Serial.println("+------------------------------------------------+");
  Serial.printf ("|   Sensor  : DHT%d on GPIO%d                     |\n", DHT_TYPE, DHT_PIN);
  Serial.printf ("|   Interval: every %lus                          |\n", READ_INTERVAL_MS / 1000);
  Serial.println("+------------------------------------------------+");
  Serial.println();
...
void loop() {
  ...
  if (isnan(temperature) || isnan(humidity)) {
    ...
    if (failCount >= 3) {
      Serial.println("  [ERROR] 3 consecutive failures — check wiring on GPIO4!");
      failCount = 0;
    }
    return;
  }
  ...
  bool newMinTemp = false, newMaxTemp = false;
  bool newMinHum  = false, newMaxHum  = false;

  if (temperature < minTemp) { minTemp = temperature; newMinTemp = true; }
  if (temperature > maxTemp) { maxTemp = temperature; newMaxTemp = true; }
  if (humidity    < minHumidity) { minHumidity = humidity; newMinHum = true; }
  if (humidity    > maxHumidity) { maxHumidity = humidity; newMaxHum = true; }

  ComfortResult comfort = classifyComfort(heatIndex, humidity);

  Serial.println();
  Serial.println("+------------------------------------------------+");
  Serial.printf ("| Read #%-4d  |  Uptime: %-18s   \n", readCount, uptime().c_str());
  Serial.println("+------------------------------------------------+");

  Serial.printf ("| Temperature  : %5.1f C", temperature);
  if (newMaxTemp && readCount > 1)      Serial.print("  ^ NEW HIGH");
  else if (newMinTemp && readCount > 1) Serial.print("  v NEW LOW ");
  else                                  Serial.print("           ");
  Serial.println("        ");

  Serial.printf ("| Feels like   : %5.1f C                        \n", heatIndex);
  Serial.printf ("| Humidity     : %-16s                \n", humidityBar(humidity).c_str());

  Serial.println("+------------------------------------------------+");
  Serial.printf ("| Status  : %-38s|\n", comfort.label.c_str());
  Serial.printf ("| Advice  : %-38s|\n", comfort.advice.c_str());
  Serial.println("+------------------------------------------------+");
  Serial.printf ("| Session min  : %5.1f C   max : %5.1f C        \n", minTemp, maxTemp);
  Serial.printf ("| Humid  min   : %5.1f %%   max : %5.1f %%        \n", minHumidity, maxHumidity);
  Serial.printf ("| Total reads  : %-4d                            \n", readCount);
  Serial.println("+------------------------------------------------+");

  unsigned long uptimeSec = (millis() - sessionStart) / 1000;
  Serial.printf("DATA,%lu,%.1f,%.1f,%.1f,%s\n",
                uptimeSec, temperature, humidity,
                heatIndex, comfort.label.c_str());
